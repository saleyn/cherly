...
hash_init (Hmap *h, int32 datasize, int64 hint)
{
	...
	datasize = runtime_rnd(datasize, sizeof (void *));
...
hash_iter_init (MapType *t, Hmap *h, struct hash_iter *it)
{
	...
	// do want top bits.
	if(sizeof(void*) == 8)
		it->cycle = (uint64)runtime_fastrand1()<<33 | (uint64)runtime_fastrand1()<<2;
	else
		it->cycle = runtime_fastrand1()<<1;
	it->cycled = false;
...
Hmap*
runtime_makemap_c(MapType *typ, int64 hint)
{
	Hmap *h;
	...
	if(valsize_in_hash >= sizeof(void*))
		h->valoff = runtime_rnd(key->size, sizeof(void*));
...
/*
	if(debug) {
		printf("makemap: map=%p; keysize=%lu; valsize=%lu; keyalg=%p; valalg=%p\n",
...
void
runtime_mapaccess(MapType *t, Hmap *h, byte *ak, byte *av, bool *pres)
{
	byte *res;
	Type *elem;
...
int32
runtime_mapassign(MapType *t, Hmap *h, byte *ak, byte *av)
{
	byte *res;
	int32 hit;

	if(av == nil) {
		hit = hash_remove(t, h, ak);
		if(debug) {
			printf("mapassign: map=%p key=%s",(void*)h , ((String*)ak)->str);
		}
		return hit;
	}
	...
	if(!hit && h->indirectval)
		*(void**)(res+h->valoff) = malloc(t->elem->size);
	t->key->alg->copy(t->key->size, res, ak);
	t->elem->alg->copy(t->elem->size, hash_indirect(h, res+h->valoff), av);

	if(debug) {
		printf("mapassign: map=%p key=%s val=%s",(void*)h, ((String*)ak)->str, ((String*)av)->str);
	}
	return hit;
}
...
void
runtime_mapiterinit(MapType *t, Hmap *h, struct hash_iter *it)
{
	if(h == nil) {
		it->data = nil;
		return;
...
void
runtime_mapiternext(struct hash_iter *it)
{
	it->data = hash_next(it);
}

bool
runtime_mapiterkey(struct hash_iter *it, void *ak)
{
	byte *res;
	Type *key;
...
void
runtime_mapiterkeyvalue(struct hash_iter *it, void *ak, void *av) 
{
	Hmap *h;
	byte *res;
	MapType *t;
